...
def urlparse(uri):
    ...
    if uri.startswith('wss://'):
        protocol = 'wss'
        rest = uri[6:]  # Remove 'wss://'
    elif uri.startswith('ws://'):
        protocol = 'ws'
        rest = uri[5:]  # Remove 'ws://'
    else:
    ...
    # Extract port from authority if present
    port_start = authority.find(':')
    if port_start == -1:
        host = authority
        port = 443 if protocol == 'wss' else 80
    else:
        host = authority[:port_start]
        port = int(authority[port_start+1:])
